...
function NFC() {
    ...
    var pub = {
        ...
        }, "getdevman": function (device, options, cb) {
            return dev_manager;
    ...
    function enumerated(d, acr122) {
        ...
        if (d && d.length != 0) {
            ...
            if (d.length)
                ...
                nDevice = 1;
            console.log(UTIL_fmt("Enumerated " + nDevice + " devices"));
            console.log(d);
        } else {
            if (d) {
            ...
            (function (dev, i) {
                window.setTimeout(function () {
                    ...
                    dev.open()
                        ...
                        .then(() => {
                            var configurationInterfaces = dev.configuration.interfaces;
                            configurationInterfaces.forEach(element => {
                                element.alternates.forEach(elementalt => {
                                    if (elementalt.interfaceClass == 0xff) {
                                        self.interfaceNumber = element.interfaceNumber;
                                        /*
        ...
        if (first) {
            window.setTimeout(function () {
                ...
                navigator.usb.requestDevice({ 'filters': filters }).then(device =>
                    ...
                    //enumerated([device], true)
                    enum_devices.push(device)
                ).then(x =>
                    enumerated(enum_devices, true)
                );
                /*
